...
public class InstituteDao extends GenericDao<Institute, Integer> implements IInstituteDao {
	...
	public Institute findInstituteNameById(Integer id) {
		...
		try {
			List results = session.createCriteria(Institute.class)
					...
					.add( Projections.property("instituteDescription"), "instituteDescription") )
	...
	public Institute findInstituteIdByName(String name) {
		...
		try {
			List results = session.createCriteria(Institute.class)
					.add(Restrictions.eq("instituteDescription", name))
					.setProjection( Projections.projectionList()
					.add( Projections.property("instituteId"), "instituteId").add( Projections.property("departmentId"), "departmentId") )
					.setResultTransformer(Transformers.aliasToBean(Institute.class))
					.list();
	...
	public HashMap<Department, List<Institute>> findAllInstitutesForAllDepartments() {
		...
		try {
			...
			for(Department dept : departments) {
				criteriaInstitute = session.createCriteria(Institute.class).add(Restrictions.eq("departmentId", dept.getDepartmentId()));
				institutes = criteriaInstitute.list();
				
				Collections.sort(institutes, (a, b) -> a.getInstituteDescription().compareToIgnoreCase(b.getInstituteDescription()));
