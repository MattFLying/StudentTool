...
/***
 * DAO class represents field of study for all operations on this type.
 * 
...
public class FieldOfStudyDao extends GenericDao<FieldOfStudy, Integer> implements IFieldOfStudyDao {
	...
	public HashMap<Department, List<FieldOfStudy>> findAllFieldsForAllDepartments() {
		...
		try {
			...
			for (Department dept : departments) {
				criteriaField = session.createCriteria(FieldOfStudy.class)
						.add(Restrictions.eq("departmentId", dept.getDepartmentId()));
				fields = criteriaField.list();

				Collections.sort(fields,
						(a, b) -> a.getFieldOfStudyName().compareToIgnoreCase(b.getFieldOfStudyName()));
		...
		try {
			...
			for (Department dept : departments) {
				...
				for (FieldOfStudy field : fields) {
					criteriaSpec = session.createCriteria(Specialization.class)
							.add(Restrictions.eq("id.fieldOfStudyId", field.getFieldOfStudyId()));
					criteriaGroup = session.createCriteria(Group.class)
							.add(Restrictions.eq("fieldOfStudyId", field.getFieldOfStudyId()));
	...
	public FieldOfStudy findFieldOfStudyNameById(Integer id) {
		...
		try {
			List results = session.createCriteria(FieldOfStudy.class).add(Restrictions.eq("fieldOfStudyId", id))
					.setProjection(Projections.projectionList()
							.add(Projections.property("fieldOfStudyName"), "fieldOfStudyName").add(
									Projections.property("fieldOfStudyId"), "fieldOfStudyId"))
	...
	public FieldOfStudy findFieldOfStudyIdByName(String name) {
		...
		try {
			List results = session.createCriteria(FieldOfStudy.class).add(Restrictions.eq("fieldOfStudyName", name))
					.setProjection(Projections.projectionList()
							.add(Projections.property("fieldOfStudyName"), "fieldOfStudyName").add(
									Projections.property("fieldOfStudyId"), "fieldOfStudyId"))
					.setResultTransformer(Transformers.aliasToBean(FieldOfStudy.class)).list();
